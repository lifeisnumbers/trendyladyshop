...
class UserFeedback_Frontend
{
	...
	public function __construct()
	{
		...
		require_once USERFEEDBACK_PLUGIN_DIR . 'includes/frontend/class-userfeedback-preview.php';
		...
		add_action('init', array($this, 'register_survey_preview_url'));
	}

	/**
	 * Create a virtual page to preview Surveys
	 *
	 * @return void
	 */
	public function register_survey_preview_url()
	{
		$virtual_page = new UserFeedback_Preview();
		$virtual_page->add(
			UserFeedback_Preview::$USERFEEDBACK_PREVIEW_PARAM,
			function ($page, $url) {
				$survey_id      = filter_input(INPUT_GET, 'survey_id', FILTER_SANITIZE_STRING);
				$element_id     = UserFeedback_Preview::$USERFEEDBACK_PREVIEW_ELEMENT_PREFIX . "-{$survey_id}";
				$page->body     = "<div id='{$element_id}'></div>";
				$page->template = 'page';
				$page->slug     = $url;
			}
		);
	...
	public function enqueue_styles_and_scripts_for_surveys()
	{
		...
		$is_survey_preview = userfeedback_is_previewing_survey();

		if ($wp_query->is_preview() && !$is_survey_preview) {
			return;
		}

		// Check if it's a survey preview
		if ($is_survey_preview) {

			$survey_id = filter_input(INPUT_GET, 'survey_id', FILTER_SANITIZE_STRING);

			if (!$survey_id || !current_user_can('userfeedback_create_edit_surveys')) {
			...
			if ($survey) {
				...
				$this->enqueue_frontend_scripts_for_preview($survey);
			} else {
				// Throw 404 error if survey with the given id doesn't exist
				$wp_query->set_404();
				status_header(404);
			}
	...
	public function enqueue_frontend_scripts_for_preview($survey)
	{
		...
		$localization_object = apply_filters(
			...
			array(
				...
				'use_survey'         => $survey,
				'is_preview'         => true,
				'preview_element_id' => UserFeedback_Preview::$USERFEEDBACK_PREVIEW_ELEMENT_PREFIX . "-{$survey->id}",
				'addons'             => array(),
