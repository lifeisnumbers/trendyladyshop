...
function userfeedback_is_previewing_survey()
{
	return isset($_GET[UserFeedback_Preview::$USERFEEDBACK_PREVIEW_PARAM]);
}
...
function userfeedback_get_frontend_widget_settings()
{
	...
	return array(
		...
		'show_logo'       => $userfeedback_settings['widget_show_logo'],
		'position'        => $userfeedback_settings['widget_position'],
		'custom_logo'     => $use_custom_logo ? $custom_logo : '',
	);
}
...
if (!function_exists('userfeedback_get_term')) {
	function userfeedback_get_term()
	{
		...
		return ($object && $object->term_id) ? $object->term_id : false;
	}
}
